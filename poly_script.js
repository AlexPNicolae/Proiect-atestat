let n = 7, Bound = 20, solutionId = 0, solOpen = false, 
    GRAPH_COLOR = 'red', has_derivative = false;
const coef = [0, -8, 0, 14, 0, -7, 0, 1], rad = [], TANGENT_LENGTH = 6;
...
document.getElementById('derivative-btn').addEventListener('click', () => {
...
function draw_point()
{
    ...
    if(has_derivative) {
        const deriv = derivataPolinom(coef, n, point.x);
        const dif = TANGENT_LENGTH / (2 * Math.sqrt(1 + deriv * deriv));
        const x1 = point.x - dif, x2 = point.x + dif;
        const y1 = point.y - deriv * dif, y2 = point.y + deriv * dif;

        ctx.strokeStyle = '#241a60';
        ctx.beginPath();
